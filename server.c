			 ...
			 * Error checks for in-session and non-existant account
				...
				printf("L1: %d, len1: %d\n", L, len1);
                		...
                		if((rval = start_account(person)) == 1) {
                    			printf("Duplicate Session for %s...please re-start your connection\n", accname);
                    			return;
                		}
				else if(rval == -1) {
...
int start_account(char* name) {
	...
	if((rval = bank_lookup(name)) == -1) {
		char find[] = "Could not find account in bank. Please refresh your connection and try using 'open'\n";
